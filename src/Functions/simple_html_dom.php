...
function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT,$unicode=false) {
    ...
    $contents = @file_get_contents($url, false, $context, $offset);
    //$contents = file_get_contents($url, $use_include_path, $context, $offset);
    if($unicode){
        $contents = iconv($unicode,'utf-8',$contents);
    }
    // Paperg - use our own mechanism for getting the contents as we want to control the timeout.
    //$contents = retrieve_url_contents($url);
    if (empty($contents) || strlen($contents) > MAX_FILE_SIZE || !$contents){
        return false;
    }
...
class simple_html_dom_node {
    ...
    function find($selector, $idx=null, $lowercase=false,$test = false)
    {
        $selectors = $this->parse_selector($selector,$test);
    ...
    protected function parse_selector($selector_string,$test = false) {
        global $debugObject;
        if (is_object($debugObject)) {$debugObject->debugLogEntry(1);}
...
class simple_html_dom
{
    ...
    {
        return $this->root->find($selector, $idx, $lowercase,1);
    }
